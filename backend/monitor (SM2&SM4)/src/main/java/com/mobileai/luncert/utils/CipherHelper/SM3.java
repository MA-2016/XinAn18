...
//		System.out.println("");
	...
	private static int[][] expand(int[] B) {
		int W[] = new int[68];
		int W1[] = new int[64];
		for (int i = 0; i < B.length; i++) W[i] = B[i];
	...
	private static int FFj(int X, int Y, int Z, int j) {
		if (j >= 0 && j <= 15) return FF1j(X, Y, Z);
		else return FF2j(X, Y, Z);
	}

	private static int GGj(int X, int Y, int Z, int j) {
		if (j >= 0 && j <= 15) return GG1j(X, Y, Z);
		else return GG2j(X, Y, Z);
	}
	...
	public static byte[] padding(byte[] in, int bLen) {
		...
		if (k < 0) k = 960 - (8 * in.length + 1) % 512;
		k += 1;
	...
	private static byte[] back(byte[] in) {
		...
		for (int i = 0; i < out.length; i++) out[i] = in[out.length - i - 1];
		return out;
	}
	...
	private static int bitCycleLeft(int n, int bitLen) {
		...
		int len = bitLen % 8;
		if (byteLen > 0) tmp = byteCycleLeft(tmp, byteLen);

		if (len > 0) tmp = bitSmall8CycleLeft(tmp, len);
