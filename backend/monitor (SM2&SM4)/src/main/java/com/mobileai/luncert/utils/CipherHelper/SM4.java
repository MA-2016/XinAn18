...
public class SM4 {
	...
	public void sm4_setkey_enc(SM4_Context ctx, byte[] key) throws Exception {
		if (ctx == null) throw new Exception("ctx is null!");

		if (key == null || key.length != 16) throw new Exception("key error!");
	...
	public byte[] sm4_crypt_ecb(SM4_Context ctx, byte[] input) throws Exception {
		...
		if (ctx.isPadding && ctx.mode == SM4_DECRYPT) output = padding(output, SM4_DECRYPT);
		bins.close();
		bous.close();
		return output;
	}
	...
	public byte[] sm4_crypt_cbc(SM4_Context ctx, byte[] iv, byte[] input) throws Exception {
		...
		if (ctx.mode == SM4_ENCRYPT) {
			for(; length > 0; length -= 16) {
				...
				bins.read(in);
				for (i = 0; i < 16; i++) out[i] = ((byte) (in[i] ^ iv[i]));
